...
5. Observability: structured logs and consistent error responses.
...
- `@UseGuards(RolesGuard)` + `@Roles('student' | 'professor')` where role matters
- Attach `req.user = { id, email, role }` for downstream use

### RBAC
- `@Roles('student')`, `@Roles('professor')`
